on("ready", function(){
     ...
     * @param string str
     * @returns int[]
     * parse string "1,2,4-5" to [1, 2, 4, 5]
     */
    function parseString2Array(str){
        var myArray = str.split(",");
        var returnArray = [];
        for (var i = 0; i != myArray.length; ++i) {
            if (myArray[i].search("-") != -1) {
                var startAndEnd = myArray[i].split("-");
                for (var j = parseInt(startAndEnd[0]); j != parseInt(startAndEnd[1]) + 1; ++j) {
                    returnArray.push(j);
                }
            } else {
                returnArray.push(parseFloat(myArray[i]));
            }
        }
        return returnArray;
    }
