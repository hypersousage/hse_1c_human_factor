on("ready", function(){
    ...
    Settings.prototype = {
        init: function(display, internal, fringeFactor, count, enabled, iffactor){
            ...
            if (!iffactor) {
                ...
                label = elem("<label>", {textContent: "Through Traffic Factor"});
            ...
            if (iffactor) {
                ...
                this.age = elem("<input>", {type: "number", min: 18, max: 80, step: 1, value: -1});
                label.append(this.age);
                options.append(label);

                label = elem("<label>", {textContent: "Sex", title: ""});
                this.sex = elem("<input>", {type: "number", min: 0, max: 1, step: 1, value: -1});
                label.append(this.sex);
                options.append(label);

                label = elem("<label>", {textContent: "Education", title: ""});
                this.edu = elem("<input>", {type: "number", min: 0, max: 1, step: 1, value: -1});
                label.append(this.edu);
                options.append(label);

                label = elem("<label>", {textContent: "Stress", title: ""});
                this.stress = elem("<input>", {type: "number", min: 0, max: 1, step: .1, value: -1});
                label.append(this.stress);
                options.append(label);

                label = elem("<label>", {textContent: "Social deviation", title: ""});
                this.socdev = elem("<input>", {type: "number", min: 0, max: 2, step: 1, value: -1});
        ...
        toJSON: function(){
            if(this.enable.checked){
                if (!this.iffactor) {
                    ...
                    console.log(this)
                    return {
                        age: parseFloat(this.age.value),
                        sex: parseFloat(this.sex.value),
                        education: parseFloat(this.edu.value),
                        stress: parseFloat(this.stress.value),
                        socdev: parseFloat(this.socdev.value)
                    };
                }
            }
    ...
    var vehicleClasses = [
        new Settings("Cars", "passenger", 5, 12, true, false),
        new Settings("Pedestrians", "pedestrian", 1, 10, false, false),
        new Settings("FACTORS", "factors", 0, 0, false, true)
    ];
