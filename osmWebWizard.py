...
class Builder(object):
    ...
    def build(self):
        ...
        if self.data["vehicles"] or ptOptions:
            ...
            factors = [];
            if "factors" in self.data["vehicles"]:
                factors = copy.deepcopy(self.data["vehicles"]["factors"])
                self.data["vehicles"].pop("factors")
            ...
            for vehicle, options in self.data["vehicles"].items():
                ...
                try:
                    options = self.parseTripOpts(vehicle, options, self.data["publicTransport"], factors)
    ...
    def parseTripOpts(self, vehicle, options, publicTransport, factors):
